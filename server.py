...
def verify_email_smtp(email: str) -> dict:
    """Verify email exists via SMTP check"""
    ...
    try:
        ...
        return {"valid": False, "reason": "Domain has no mail server. This email cannot receive mail."}
    ...
    try:
        ...
        if code == 250:
            return {"valid": True, "reason": "Email accepted by mail server"}
        elif code == 550:
            return {"valid": False, "reason": "Email address does not exist"}
        else:
            # Inconclusive — allow registration but mail server didn't confirm
            return {"valid": True, "reason": "Mail server responded but could not confirm"}
    except Exception as e:
        # If SMTP fails, fall back to MX check only
        # MX exists so domain is valid, we just can't verify the mailbox
...
**Micronutrients:**
- Vitamin D: 600-800 IU daily (crucial for bone health)
- Iron: 8mg (men), 18mg (women) daily
- Calcium: 1000-1200mg daily
- Omega-3: 250-500mg EPA+DHA daily
...
**Special Diets:**
- Mediterranean diet: Associated with heart health benefits
- Plant-based: Ensure B12, iron, zinc supplementation
- Low-carb/Keto: Effective for weight loss; consult doctor first
- Intermittent fasting: 16:8 most common; not suitable for everyone
...
- Practice mindful eating; eat slowly

**Supplements:**
- Most people don't need supplements with balanced diet
...
- Cool, dark room optimal for sleep
...
- Social fitness activities boost adherence
...
- Maintain healthy waist circumference (<94cm men, <80cm women)
...
SCRAPE_SOURCES = {
    "WORKOUT": [
        ...
        {"url": "https://health.gov/our-work/nutrition-physical-activity/physical-activity-guidelines", "name": "Health.gov Physical Activity"},
    ],
    "NUTRITION": [
        {"url": "https://www.who.int/news-room/fact-sheets/detail/healthy-diet", "name": "WHO Healthy Diet"},
        {"url": "https://www.cdc.gov/nutrition/php/data-research/index.html", "name": "CDC Nutrition"},
...
async def scrape_url(url: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            headers = {
                "User-Agent": "Mozilla/5.0 (compatible; FitnessAI/1.0; Health Research Bot)"
            }
...
async def get_scraped_knowledge(intent: str) -> str:
    """Get scraped knowledge from cache or scrape fresh"""
    cache_key = f"scrape_cache_{intent}"
    cached = await db.scrape_cache.find_one(
...
def generate_workout_pdf(workout: dict, user_name: str = "User") -> bytes:
    ...
    pdf.set_text_color(161, 161, 170)
    pdf.cell(60, 8, "Exercise", border=1, fill=True)
    pdf.cell(25, 8, "Sets", border=1, fill=True, align="C")
    pdf.cell(30, 8, "Reps", border=1, fill=True, align="C")
    pdf.cell(25, 8, "Rest", border=1, fill=True, align="C")
    pdf.cell(45, 8, "Muscle Group", border=1, fill=True, align="C")
    pdf.ln()
...
async def get_user_profile_context(user_id: str) -> str:
    ...
    if profile.get("dietary_preference"):
        parts.append(f"Dietary Preference: {profile['dietary_preference']}")
    if parts:
        return "\n\nUser Profile:\n" + "\n".join(parts)
    return ""
...
async def generate_rag_response(message: str, intent: str, user_id: str) -> str:
    try:
        # For off-topic questions, give a short redirect response
        if intent == "OTHER":
            system_msg = """You are FitnessAI, a fitness and health coach. The user asked something outside your scope (not about workouts, nutrition, diet, or health). Respond in exactly 1-2 sentences: politely say you only help with fitness, nutrition, and health topics, then suggest one relevant fitness question they could ask instead. Do NOT give a long answer. Do NOT use headings, bullet points, or sections. Keep it under 40 words."""
        ...
        # Fetch real-time scraped knowledge
...
- Always structure your response with clear sections using markdown headings (## and ###)
- Use bullet points (-) for lists, never plain text paragraphs for multiple items
- Put key numbers, sets, reps, calories in **bold**
- Use tables (| col1 | col2 |) when comparing options or showing structured data like meal plans or exercise sets
...
async def register(req: RegisterRequest, response: Response):
    ...
    # Step 2: Check if it's a Gmail address (restrict to Gmail)
    domain = email.split('@')[1]
    gmail_domains = ['gmail.com', 'googlemail.com']
    if domain not in gmail_domains:
        raise HTTPException(status_code=400, detail="Only Gmail addresses are allowed. Please use a @gmail.com email.")
    
    # Step 3: Verify email exists via DNS/SMTP
...
async def update_profile(profile: ProfileUpdate, request: Request):
    ...
    updated = await db.user_profiles.find_one({"user_id": user_id}, {"_id": 0})
    return updated
...
async def send_chat_message(chat_request: ChatRequest, request: Request):
    ...
    await db.chat_messages.insert_one({
        "message_id": user_msg_id, "user_id": user_id,
        "chat_session_id": chat_session_id, "role": "user",
        "content": chat_request.message, "intent": None,
    ...
    await db.chat_messages.insert_one({
        "message_id": ai_msg_id, "user_id": user_id,
        "chat_session_id": chat_session_id, "role": "assistant",
        "content": ai_response, "intent": intent,
        "timestamp": datetime.now(timezone.utc).isoformat()
    })

    return ChatResponse(
        message_id=ai_msg_id, content=ai_response,
        intent=intent, chat_session_id=chat_session_id
    )
...
async def get_chat_sessions(request: Request):
    ...
    pipeline = [
        ...
        {"$project": {
            "_id": 0, "chat_session_id": "$_id",
            "last_message": 1, "last_timestamp": 1,
            "message_count": 1, "first_message": 1
        }}
    ]
...
[
  {{"meal_type": "Breakfast", "name": "...", "description": "...", "calories": 350, "protein": 20, "carbs": 40, "fats": 12}},
  {{"meal_type": "Morning Snack", "name": "...", "description": "...", "calories": 150, "protein": 10, "carbs": 15, "fats": 5}},
  {{"meal_type": "Lunch", "name": "...", "description": "...", "calories": 500, "protein": 35, "carbs": 50, "fats": 15}},
  {{"meal_type": "Afternoon Snack", "name": "...", "description": "...", "calories": 150, "protein": 8, "carbs": 18, "fats": 5}},
  {{"meal_type": "Dinner", "name": "...", "description": "...", "calories": 450, "protein": 30, "carbs": 45, "fats": 14}}
]"""
        ).with_model("gemini", "gemini-3-flash-preview")
...
async def get_meal_plans(request: Request):
    user_id = await get_current_user(request)
    return await db.meal_plans.find(
...
Return ONLY valid JSON (no markdown fences) in this exact format:
[
  {{"name": "Exercise Name", "sets": 3, "reps": "12", "rest": "60s", "muscle_group": "Chest", "tips": "Keep back straight"}},
  ...
]
Include 5-7 exercises. Use "reps" for count-based and time-based (e.g., "30s") exercises."""
        ).with_model("gemini", "gemini-3-flash-preview")
...
async def add_progress(entry: ProgressEntry, request: Request):
    ...
    doc = {
        ...
        "weight": entry.weight, "bmi": entry.bmi,
        "body_fat": entry.body_fat, "muscle_mass": entry.muscle_mass,
...
async def get_progress(request: Request):
    user_id = await get_current_user(request)
    return await db.progress_metrics.find(
        {"user_id": user_id}, {"_id": 0}
...
async def export_meal_pdf(plan_id: str, request: Request):
    ...
    pdf_bytes = generate_meal_pdf(plan, user_name)
...
async def export_workout_pdf(workout_id: str, request: Request):
    ...
    workout = await db.workouts.find_one(
        {"workout_id": workout_id, "user_id": user_id}, {"_id": 0}
    )
    if not workout:
        raise HTTPException(status_code=404, detail="Workout not found")
    ...
    pdf_bytes = generate_workout_pdf(workout, user_name)
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
...
# ============ RAG Knowledge Scraping Endpoints ============

@api_router.post("/rag/refresh")
async def refresh_rag_cache(request: Request):
    """Manually refresh scraped knowledge cache"""
...
async def get_rag_status(request: Request):
    """Get status of RAG knowledge sources"""
    await get_current_user(request)
    statuses = []
    for intent, sources in SCRAPE_SOURCES.items():
        cached = await db.scrape_cache.find_one(
            {"cache_key": f"scrape_cache_{intent}"}, {"_id": 0}
        )
        statuses.append({
            "intent": intent,
            "sources": [s["name"] for s in sources],
